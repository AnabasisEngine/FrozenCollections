...
public readonly struct FrozenHashTable
{
    ...
    public static FrozenHashTable Create<T>(IReadOnlyList<T> entries, Func<T, int> hasher, Action<int, T> setter)
    {
        ...
        else
        {
            ...
            var numBuckets = CalcNumBuckets(hashCodes);
            ...
            for (int index = 0; index < numEntries; index++)
            {
                ...
                if (!chainBuddies.TryGetValue(bucket, out var list))
                {
            ...
            int count = 0;
            foreach (var list in chainBuddies.Values)
            {
    ...
    private static int CalcNumBuckets(int[] hashCodes)
    {
        ...
        var codes = new HashSet<int>(hashCodes).ToArray();
        ...
        for (int numBuckets = codes.Length; numBuckets < maxNumBuckets; numBuckets++)
        {
            ...
            {
                var bucketNum = (uint)code % (uint)numBuckets;
